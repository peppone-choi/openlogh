...
레거시 PHP에서 추출된 게임 정적 리소스가 새 프로젝트에 존재하고 올바른지 검증합니다:
...
- `backend/game-app/src/main/resources/data/` 하위 파일을 변경한 후
...
| File                                                 | Purpose                                 |
| ---------------------------------------------------- | --------------------------------------- |
| `legacy-core/hwe/sammo/Scenario.php`                      | 레거시 시나리오 로더                    |
| `legacy-core/hwe/sammo/CityConstBase.php`                 | 레거시 도시 상수 (100+ 도시)            |
| `legacy-core/hwe/sammo/GameConstBase.php`                 | 레거시 게임 상수                        |
| `legacy-core/hwe/sammo/GameUnitConstBase.php`             | 레거시 병종 상수                        |
| `legacy-core/hwe/sammo/GeneralBuilder.php`                | 레거시 장수 빌더 (NPC 생성)             |
| `legacy-core/hwe/sammo/Nation.php`                        | 레거시 국가 빌더                        |
| `backend/game-app/src/main/resources/data/game_const.json`    | 도시 레벨/지역 매핑, 초기값 (구현 대상) |
| `backend/game-app/src/main/resources/data/officer_ranks.json` | 관직 체계 (구현 대상)                   |
| `backend/game-app/src/main/resources/data/maps/`              | 맵 데이터 (구현 대상)                   |

## Workflow

### Step 1: 시나리오 데이터 존재 확인

**파일:** `backend/game-app/src/main/resources/data/scenarios/` 또는 동등 경로

**검사:** 레거시 24종 시나리오에 대응하는 데이터가 존재하는지 확인합니다.

```bash
# 새 프로젝트 시나리오 파일 목록
ls backend/game-app/src/main/resources/data/scenarios/ 2>/dev/null | wc -l

# 레거시 시나리오 참조 (Scenario.php에서 시나리오 ID 추출)
grep -n "scenario_\|getAllScenarios\|scenarioList" legacy-core/hwe/sammo/Scenario.php | head -20
```
...
**파일:** `backend/game-app/src/main/resources/data/maps/`

**검사:** 8종 맵 데이터가 존재하는지 확인합니다.

```bash
ls backend/game-app/src/main/resources/data/maps/ 2>/dev/null | sort
```
...
**파일:** `backend/game-app/src/main/resources/data/game_const.json`

**검사:** game_const.json에 도시 정의가 포함되어 있고, 레거시 CityConstBase의 도시 수와 일치하는지 확인합니다.

```bash
# game_const.json 존재 및 도시 수 확인
if [ -f backend/game-app/src/main/resources/data/game_const.json ]; then
  echo "PASS: game_const.json exists"
  # 도시 항목 수 대략 확인
  grep -c '"city":\|"cityId":\|"name":' backend/game-app/src/main/resources/data/game_const.json 2>/dev/null || echo "구조 확인 필요"
else
...
grep -c "new CityInitialDetail\|'region'" legacy-core/hwe/sammo/CityConstBase.php 2>/dev/null
```
...
**파일:** `backend/game-app/src/main/resources/data/officer_ranks.json`

**검사:** 관직 데이터가 존재하고, CLAUDE.md에 정의된 관직 체계(국가 레벨 0~7)를 포함하는지 확인합니다.

```bash
if [ -f backend/game-app/src/main/resources/data/officer_ranks.json ]; then
  echo "PASS: officer_ranks.json exists"
  # 국가 레벨별 관직 존재 확인
  grep -c '"level":\|"rank":\|"승상\|태위\|사도\|사공"' backend/game-app/src/main/resources/data/officer_ranks.json 2>/dev/null || echo "구조 확인 필요"
else
...
grep -rn "crewType\|unitType\|병종\|GameUnit" backend/game-app/src/main/resources/data/ 2>/dev/null
grep -rn "crewType\|unitType" backend/game-app/src/main/kotlin/com/opensam/ 2>/dev/null
```
