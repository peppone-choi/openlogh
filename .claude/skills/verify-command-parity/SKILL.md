...
5. **진행 상태 동기화** — 레거시 커맨드 목록 대비 구현/미구현 상태를 실제 코드 기준으로 정리
...
| File                                                                  | Purpose                           |
| --------------------------------------------------------------------- | --------------------------------- |
| `legacy-core/hwe/sammo/Command/General/*.php`                              | 레거시 장수 커맨드 PHP 소스       |
| `legacy-core/hwe/sammo/Command/Nation/*.php`                               | 레거시 국가 커맨드 PHP 소스       |
| `backend/game-app/src/main/kotlin/com/opensam/command/general/*.kt`            | 백엔드 장수 커맨드 (구현 대상)    |
| `backend/game-app/src/main/kotlin/com/opensam/command/nation/*.kt`             | 백엔드 국가 커맨드 (구현 대상)    |
| `backend/game-app/src/main/kotlin/com/opensam/command/CommandExecutor.kt`      | 커맨드 실행기 (라우팅/디스패치)   |
| `backend/game-app/src/main/kotlin/com/opensam/controller/CommandController.kt` | 커맨드 REST 엔드포인트            |
| `backend/game-app/src/main/kotlin/com/opensam/service/CommandService.kt`       | 커맨드 서비스 레이어              |
| `backend/game-app/src/main/kotlin/com/opensam/dto/CommandDtos.kt`              | 커맨드 요청/응답 DTO              |
| `frontend/src/components/game/*.tsx`                                  | 프론트엔드 커맨드 UI (구현 대상)  |
| `frontend/src/app/(game)/commands/page.tsx`                           | 커맨드 화면 진입점               |

## Workflow

### Step 1: 레거시 커맨드 카탈로그 로드

**파일:** `legacy-core/hwe/sammo/Command/General/*.php`, `legacy-core/hwe/sammo/Command/Nation/*.php`

**검사:** 레거시 PHP 커맨드 디렉토리에서 기준 목록을 구축합니다.

```bash
ls legacy-core/hwe/sammo/Command/General/*.php 2>/dev/null | wc -l
ls legacy-core/hwe/sammo/Command/Nation/*.php 2>/dev/null | wc -l
```
...
**파일:** `backend/game-app/src/main/kotlin/com/opensam/command/general/`

**검사:** 각 장수 커맨드 키에 대응하는 Kotlin 클래스가 존재하는지 확인합니다.

```bash
ls backend/game-app/src/main/kotlin/com/opensam/command/general/ 2>/dev/null | sort
```
...
**PASS:** 구현된 커맨드 파일이 존재
**FAIL:** 커맨드 파일이 누락됨 — 구현 필요 또는 미구현 목록으로 보고

### Step 3: 백엔드 국가 커맨드 존재 확인

**파일:** `backend/game-app/src/main/kotlin/com/opensam/command/nation/`

**검사:** 각 국가 커맨드 키에 대응하는 Kotlin 클래스가 존재하는지 확인합니다.

```bash
ls backend/game-app/src/main/kotlin/com/opensam/command/nation/ 2>/dev/null | sort
```
...
grep -rn "register\|commandMap\|CommandType" backend/game-app/src/main/kotlin/com/opensam/command/ 2>/dev/null
```
...
**파일:** `frontend/src/components/game/`, `frontend/src/app/(game)/commands/page.tsx`, `frontend/src/types/`
...
ls legacy-core/hwe/sammo/Command/General/*.php 2>/dev/null | wc -l
ls legacy-core/hwe/sammo/Command/Nation/*.php 2>/dev/null | wc -l
ls backend/game-app/src/main/kotlin/com/opensam/command/general/*.kt 2>/dev/null | wc -l
ls backend/game-app/src/main/kotlin/com/opensam/command/nation/*.kt 2>/dev/null | wc -l
```
...
1. **미구현 커맨드** — 아직 구현되지 않은 상태는 패러티 진행 상황으로 보고하고, 즉시 FAIL로 고정하지 않음
