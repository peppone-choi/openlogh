...
| `legacy-core/hwe/scenario/scenario_*.json`          | 레거시 시나리오 장수 데이터 (3-stat)   |
| `legacy-core/hwe/scenario/frame.json`               | 시나리오 프레임/기본 구조              |
| `legacy-core/hwe/sammo/Scenario/GeneralBuilder.php` | 레거시 장수 빌더 (3-stat)              |
| `legacy-core/hwe/sammo/Scenario.php`                | 레거시 시나리오 로더                   |
| `backend/game-app/src/main/resources/data/**/*`        | 새 게임 데이터 리소스 (구현 대상)     |

## Workflow

### Step 1: 새 프로젝트 시나리오 데이터 형식 확인

**검사:** 새 프로젝트의 게임 데이터 리소스에 5-stat 장수 데이터가 반영되었는지 확인합니다.

```bash
# 새 프로젝트 데이터 리소스 존재 확인
ls backend/game-app/src/main/resources/data/ 2>/dev/null

# 장수 데이터에서 politics/charm 필드 존재 확인
grep -rn "politics\|charm" backend/game-app/src/main/resources/data/ 2>/dev/null
```
...
**파일:** `legacy-core/hwe/scenario/scenario_*.json`

**검사:** 레거시 시나리오의 장수 수와 3-stat 구조를 파악합니다.

```bash
# 레거시 시나리오 수
ls legacy-core/hwe/scenario/scenario_*.json | wc -l

# 샘플 장수 데이터 (첫 시나리오의 general 배열)
python3 -c "
import json
with open('legacy-core/hwe/scenario/scenario_1010.json') as f:
    data = json.load(f)
...
import json, glob
names = set()
for f in glob.glob('legacy-core/hwe/scenario/scenario_*.json'):
    with open(f) as fh:
        data = json.load(fh)
...
```bash
# 비역사 NPC 공식 관련 코드 검색
grep -rn "generatePolitics\|generateCharm\|calcPolitics\|calcCharm\|formulaPolitics\|formulaCharm\|nonHistorical\|virtualGeneral" backend/game-app/src/main/kotlin/com/opensam/ 2>/dev/null

# 가상 장수 식별 로직 검색
grep -rn "isHistorical\|isVirtual\|isFictional\|삼국지14" backend/game-app/src/main/kotlin/com/opensam/ 2>/dev/null
```
