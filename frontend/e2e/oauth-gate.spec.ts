...
function createTestJwt(payload: Record<string, unknown>): string {
    ...
    return [base64UrlEncode(JSON.stringify(header)), base64UrlEncode(JSON.stringify(payload)), 'sig'].join('.');
}
    ...
    test('OAuth callback success lands in lobby and allows world entry', async ({ page }) => {
        ...
        await page.route(`${API_BASE}/scenarios`, async (route) => {
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify([{ code: 'std_hero', title: '기본 시나리오', description: 'e2e' }]),
            });
        });
