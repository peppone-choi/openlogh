const legacyBaseUrl = process.env.PARITY_LEGACY_BASE_URL ?? 'http://localhost:8888';

export const LEGACY = {
    baseUrl: legacyBaseUrl,
    gameUrl: process.env.PARITY_LEGACY_GAME_URL ?? `${legacyBaseUrl}/sam/che/`,
    credentials: {
        username: process.env.PARITY_LEGACY_USERNAME ?? 'admin',
        password: process.env.PARITY_LEGACY_PASSWORD ?? 'admin123',
    },
};

export const NEW_SYSTEM = {
    baseUrl: process.env.PARITY_NEW_BASE_URL ?? 'http://localhost:80',
    apiUrl: process.env.PARITY_NEW_API_URL ?? 'http://localhost:8080/api',
    gameAppUrl: process.env.PARITY_NEW_GAME_APP_URL ?? 'http://localhost:9001',
    credentials: {
        loginId: process.env.PARITY_NEW_LOGIN_ID ?? 'admin',
        password: process.env.PARITY_NEW_PASSWORD ?? process.env.ADMIN_PASSWORD ?? 'CHANGE_ME_ADMIN_PASSWORD',
    },
};
...
export const GENERAL_COMMANDS: CommandItem[] = [
    ...
    },
    {
        ...
        category: 'civil',
        safeToExecute: true,
        effectChecks: ['general', 'city', 'nation'],
    },
    {
        actionCode: '군량매매',
        category: 'civil',
...
export const SAFE_GENERAL_EXECUTION_CODES = GENERAL_COMMANDS.filter((command) => command.safeToExecute).map(
    (command) => command.actionCode
);

export const SAFE_NATION_EXECUTION_CODES = NATION_COMMANDS.filter((command) => command.safeToExecute).map(
    (command) => command.actionCode
);
