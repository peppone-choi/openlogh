...
test.describe('Parity: Main', () => {
    test('main page sections are visible in legacy and new', async ({ browser }) => {
        ...
        try {
            ...
            const legacyMapVisible = (await legacyPage.locator('img[usemap], map area, .city').count()) > 0;
            const newMapVisible = (await nextPage.locator("canvas, [data-city-id], [class*='map']").count()) > 0;
            results.push({
