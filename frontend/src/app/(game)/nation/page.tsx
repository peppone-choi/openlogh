...
function NationPageContent() {
    ...
    const loadNationData = useCallback(async () => {
        ...
        const extra = off
            ? [nationPolicyApi.getPolicy(nId), diplomacyApi.listByNation(wId, nId), nationApi.listByWorld(wId)]
            : [];

        try {
            const res = await Promise.all([...base, ...extra]);
            const nationData = (res[0] as { data: Nation }).data;
    ...
    const cityOfficers = useMemo(() => {
        const m = new Map<number, { taesu?: General; gunsa?: General; jongsa?: General }>();
        generals.forEach((g) => {
