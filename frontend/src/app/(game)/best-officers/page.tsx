        ...
        categories: [
            {
                ...
                label: '총합',
                sortBy: 'experience',
                getValue: (g) => g.leadership + g.strength + g.intel + g.politics + g.charm,
            },
            {
                key: 'leadership',
                label: '통솔',
    ...
    },
    {
        ...
        categories: [
            ...
            },
            {
                key: 'killnum_person',
                label: '대인사살',
                sortBy: 'killnum_person',
                getValue: (g) => m(g, 'killnum_person'),
            },
            {
                key: 'killrate_person',
                label: '대인살상률',
                sortBy: 'killrate_person',
                valueType: 'percent',
                getValue: (g) => {
                    const kp = m(g, 'killnum_person');
                    const dp = m(g, 'deathnum_person');
                    return m(g, 'warnum') >= 10 ? kp / Math.max(1, dp) : 0;
                },
            },
            {
                key: 'firenum',
...
const DEFAULT_CATEGORY_BY_GROUP: Record<CategoryGroup, string> = {
    ...
    combat: GROUPS.find((g) => g.key === 'combat')?.categories[0]?.key ?? 'experience',
    dex: GROUPS.find((g) => g.key === 'dex')?.categories[0]?.key ?? 'dex1',
    tournament: GROUPS.find((g) => g.key === 'tournament')?.categories[0]?.key ?? 'ttw',
...
export default function BestGeneralsPage() {
    ...
    const [categoryByGroup, setCategoryByGroup] = useState<Record<CategoryGroup, string>>(DEFAULT_CATEGORY_BY_GROUP);
    ...
    const category = group.categories.find((c) => c.key === categoryKey) ?? group.categories[0];
    ...
    return (
        <div className="p-4 space-y-4 max-w-5xl mx-auto">
                    ...
                    {(['user', 'npc'] as const).map((mode) => (
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs transition-colors ${
                                npcMode === mode ? 'bg-[#141c65] text-white' : 'text-gray-400 hover:text-white'
                            }`}
                        >
            ...
            {/* Group tabs */}
            <Tabs value={groupKey} onValueChange={(v: string) => setGroupKey(v as CategoryGroup)}>
                <TabsList>
                    {GROUPS.map((g) => (
            ...
            <Tabs
                value={categoryKey}
                onValueChange={(value: string) => setCategoryByGroup((prev) => ({ ...prev, [groupKey]: value }))}
            >
                <TabsList>
