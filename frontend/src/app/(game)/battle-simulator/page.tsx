...
const ITEM_CODES = ['', 'nothing', 'jade_seal', 'imperial_edict', 'ancient_wine', 'seven_star_sword', 'bronze_sparrow'];
const SPECIAL_CODES = [
    '',
    'none',
    'fire_attack',
        ...
        (data: Record<string, unknown>) => {
            if (data.objType === 'general' && data.data) {
                ...
                onChange({
                    ...
                    warnum: (d.warnum as number) ?? 0,
                    killnum: (d.killnum as number) ?? 0,
                    killcrew: (d.killcrew as number) ?? 0,
                    inheritBuff: defaultInheritBuff(),
                });
            }
        },
    ...
    return (
        ...
        >
            ...
            <CardContent className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                    <NumberField label="전투수" value={unit.warnum} onChange={(v) => set('warnum', v)} min={0} />
                    <NumberField label="승리" value={unit.killnum} onChange={(v) => set('killnum', v)} min={0} />
                    <NumberField
                        label="격파병"
                        value={unit.killcrew}
                        onChange={(v) => set('killcrew', v)}
                        min={0}
                        step={100}
                    />
                </div>
                    ...
                    {showInherit && (
                        <div className="mt-1 grid grid-cols-2 gap-1">
                            <NumberField
                                ...
                                onChange={(v) => set('inheritBuff', { ...unit.inheritBuff, warAvoidRatio: v })}
                            />
                            <NumberField
                                label="회피저"
...
export default function BattleSimulatorPage() {
    ...
    const skillSummary = useMemo(() => {
        ...
        return {
            attacker: ext.attackerSkills,
            defenders: ext.defendersSkills ?? [],
        };
    }, [result]);
    ...
    const handleSimulate = async () => {
        ...
        try {
            ...
            const attackerUnit: BattleSimUnit & { warnum?: number; killnum?: number; killcrew?: number } = {
                ...toUnit(attacker),
                warnum: attacker.warnum,
                killnum: attacker.killnum,
                killcrew: attacker.killcrew,
            };
            const defenderUnit: BattleSimUnit & { warnum?: number; killnum?: number; killcrew?: number } = {
                ...toUnit(defender),
                warnum: defender.warnum,
                killnum: defender.killnum,
                killcrew: defender.killcrew,
            };
    ...
    return (
        <div className="space-y-4 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-4">
                    {defenders.map((def, idx) => (
                        <div
                            key={`${def.name}-${def.crew}-${def.leadership}-${def.strength}-${def.intel}-${def.warnum}-${def.killnum}-${def.killcrew}`}
                            className="relative"
                        >
                            <UnitBuilder
            ...
            {/* Error */}
            {error && (
                <div className="text-sm text-destructive border border-red-900 bg-red-950/30 p-3 rounded">{error}</div>
            )}
            ...
            {result && (
                <Card>
                    ...
                    <CardContent className="space-y-3">
                                        ...
                                        <div className="text-lg font-bold text-red-400 tabular-nums">
                                            {(
                                                (1 -
                                                    ...
                                                    result.repeatSummary.draws / result.repeatSummary.totalRuns) *
                                                100
                                            ).toFixed(1)}
                                            %
                                        </div>
                        ...
                        {skillSummary && (skillSummary.attacker || skillSummary.defenders.length > 0) && (
                            <div className="space-y-1">
                                <div className="text-xs text-muted-foreground font-medium">스킬 카운트</div>
                                {skillSummary.attacker && (
                                    <div className="text-xs">
                                        <span className="text-muted-foreground">공격측:</span>{' '}
                                        {Object.entries(skillSummary.attacker)
                                            .map(([skill, count]) => `${skill}(${count})`)
                                            .join(', ') || '-'}
                                    </div>
                                )}
                                {skillSummary.defenders.map((skills, idx) => (
                                    <div key={JSON.stringify(skills)} className="text-xs">
                                        <span className="text-muted-foreground">방어측 {idx + 1}:</span>{' '}
