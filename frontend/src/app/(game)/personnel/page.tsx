    ...
    return (
        <div className="space-y-2">
            <div className="flex items-center justify-between">
                ...
                <Button size="sm" variant="outline" disabled={aLoading} onClick={handleSave}>
                    변경
                </Button>
            </div>
                    ...
                    return (
                        <Badge
                            ...
                            className={`cursor-pointer transition-colors ${
                                isSelected ? 'bg-purple-600 hover:bg-purple-700' : 'hover:bg-muted/50'
                            }`}
...
export default function PersonnelPage() {
    ...
    const getCandidatesForLevel = (level: number): General[] => {
        if (level === 12 || level === 11) return candidatesAny;
    ...
    return (
        <div className="p-4 space-y-4 max-w-5xl mx-auto">
                ...
                <CardContent className="space-y-2">
                    {(() => {
                        ...
                        for (let i = 12; i >= minChiefLevel; i -= 2) {
                            ...
                            rows.push(
                                <div key={i} className="grid grid-cols-2 gap-2 border-b border-muted/30 pb-2">
                    ...
                    {tigers.length > 0 && (
                        <div className="flex items-start gap-2 text-sm">
                            ...
                            <span>{tigers.map((t) => `${t.name}【${t.value.toLocaleString()}】`).join(', ')}</span>
                        </div>
                    )}
                    {eagles.length > 0 && (
                        <div className="flex items-start gap-2 text-sm">
                            ...
                            <span>{eagles.map((e) => `${e.name}【${e.value.toLocaleString()}】`).join(', ')}</span>
                        </div>
                    )}
                </CardContent>
            </Card>
                    ...
                    <CardContent className="space-y-3">
                        {[4, 3, 2].map((level) => {
                            ...
                            return (
                                ...
                                >
                                    <Badge variant="outline" className="w-16 justify-center shrink-0 text-xs">
                                        {levelLabel}
                                    </Badge>
                                    <select
                ...
                <CardContent>
                    <div className="overflow-x-auto">
                        ...
                        <p className="text-xs text-muted-foreground mt-2">
                            ※ <span className="text-orange-400">노란색</span>은 변경 불가능, 흰색은 변경 가능
                            관직입니다.
                        </p>
                    </div>
                </CardContent>
            ...
            {isKing && (
                <Card>
                    <CardHeader className="pb-2">
                        <CardTitle className="text-base text-purple-400">외교권자 / 조언자 임명</CardTitle>
                    </CardHeader>
