...
import { inheritanceApi, planetApi } from '@/lib/gameApi';
...
export default function InheritPage() {
    ...
    // Combat buffs
    const [combatBuffLevels, setCombatBuffLevels] = useState<Record<string, number>>({});
    ...
    const fetchInfo = useCallback(async () => {
        ...
        try {
            const [infoRes, cityRes] = await Promise.all([
                ...
                planetApi.listByWorld(currentWorld.id),
            ]);
    ...
    const handleResetTurn = async () => {
        ...
        if (!confirm(`턴 시간을 초기화하시겠습니까? (비용: ${turnResetCost}P)`)) return;
        try {
    ...
    const handleBuyRandomUnique = async () => {
        ...
        if (!confirm(`랜덤 유니크 아이템을 획득하시겠습니까? (비용: ${actionCost.randomUnique}P)`)) return;
        try {
    ...
    const handleResetSpecialWar = async () => {
        ...
        if (!confirm(`전투특기를 재배정하시겠습니까? (비용: ${specialWarResetCost}P)`)) return;
        try {
    ...
    const handleResetStats = async () => {
        ...
        if (
            !confirm(`능력치를 초기화 하시겠습니까? 시즌마다 한번만 가능합니다. 필요 포인트: ${requiredResetStatPoint}`)
        )
            return;
        try {
            const payload: {
    ...
    const handleCheckOwner = async () => {
        ...
        const targetName = info.availableTargetGeneral?.[Number(targetOwnerId)] ?? targetOwnerId;
        if (!confirm(`${targetName}의 소유자를 찾겠습니까? 대상에게도 알림이 전송됩니다.`)) return;
        try {
            const { data } = await inheritanceApi.checkOwner(currentWorld.id, Number(targetOwnerId));
            if (data.found) {
                setOwnerResult(`'${targetName}' 제독의 소유주: ${data.ownerName}`);
            } else {
                setOwnerResult(`'${targetName}' 제독를 찾을 수 없습니다.`);
            }
            fetchInfo();
        } catch {
    ...
    const handleAuctionUnique = async () => {
        ...
        if (!confirm(`'${uniqueName}'을(를) ${auctionBid}P로 입찰하시겠습니까?`)) return;
        try {
    ...
    const handleLoadMoreLogs = async () => {
        ...
        try {
            const { data } = await inheritanceApi.getMoreLog(currentWorld.id, lastLogID);
            if (data.log && data.log.length > 0) {
    ...
    const maxBuff = Math.min(info?.maxInheritBuff ?? MAX_BUFF_LEVEL, Math.max(actionCost.buff.length - 1, 0));
            ...
            <Card>
                ...
                <CardContent className="space-y-3">
                            ...
                            {POINT_BREAKDOWN_ORDER.map((key) => {
                                ...
                                return (
                                    <div key={key} className="flex justify-between">
                                        ...
                                        <span
                                            className={
                                                key === 'previous' ? '' : value > 0 ? 'text-green-400' : 'text-red-400'
                                            }
                                        >
                                            {key === 'previous'
                                                ? `${Math.floor(value)}P`
                        ...
                        <div className="text-xs text-muted-foreground space-y-1 border-t border-gray-800 pt-2">
                            ...
                            {info.pointSources?.map((src) => (
                                <div key={`${src.label}-${src.amount}`} className="flex justify-between">
                                    <span className="pl-2 text-muted-foreground/70">└ {src.label}</span>
                                    <span className={src.amount > 0 ? 'text-green-400' : 'text-red-400'}>
                    ...
                    <p className="text-sm text-muted-foreground">
                        이전 게임에서 획득한 포인트로 새 제독에게 보너스를 줄 수 있습니다.
                    </p>
                </CardContent>
            </Card>
            ...
            <Tabs defaultValue="combat-buffs">
                                ...
                                return (
                                    <div key={buff.code} className="border border-gray-800 rounded-md p-3 space-y-2">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <Button
                                                size="sm"
                                                disabled={!info || currentSelected <= prevLevel || info.points < cost}
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-muted-foreground w-16">목표 레벨:</span>
                                            <Input
                                                type="number"
                                                min={prevLevel}
                                                max={maxBuff}
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            ...
                            <div className="flex gap-2">
                                ...
                                <Button
                                    size="sm"
                                    disabled={!selectedSpecial || !info || info.points < actionCost.nextSpecial}
                ...
                <TabsContent value="shop" className="space-y-4">
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            <p className="text-sm text-muted-foreground">
                                다다음턴부터 시간이 랜덤하게 바뀝니다. (필요 포인트가 피보나치식으로 증가합니다)
                            </p>
                            <div className="flex items-center justify-between">
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            <p className="text-sm text-muted-foreground">
                                즉시 전투 특기를 초기화합니다. (필요 포인트가 피보나치식으로 증가합니다)
                            </p>
                            <div className="flex items-center justify-between">
                                <span className="text-xs text-muted-foreground">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center justify-between pt-1">
                                    <span className="text-xs text-muted-foreground">
                                        ...
                                        {bonusStatRemaining > 0 ? ` (남음: ${bonusStatRemaining})` : ''} · 필요 포인트:{' '}
                                        {requiredResetStatPoint}P
                                    </span>
                                ...
                                {!bonusStatValid && (
                                    <p className="text-xs text-red-400">
                                        보너스 능력치는 사용하지 않거나, 정확히 {MAX_INHERIT_BONUS_STAT_TOTAL} 포인트를
                                        분배해야 합니다.
                                    </p>
                                )}
                            </div>
                ...
                <TabsContent value="unique" className="space-y-4">
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            <p className="text-sm text-muted-foreground">다음 턴에 랜덤 유니크를 얻습니다.</p>
                            <Button
                                size="sm"
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex gap-2 items-center">
                                    <Input
                                        ...
                                        min={minBid}
                                        value={auctionBid}
                                        onChange={(e) => setAuctionBid(Math.max(minBid, Number(e.target.value)))}
                                        className="flex-1"
                                ...
                                <Button
                                    size="sm"
                                    disabled={
                                        !selectedUnique || !info || info.points < auctionBid || auctionBid < minBid
                                    }
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2 text-sm">
                                ...
                                제독 소유주 확인 ({actionCost.checkOwner}P)
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            <p className="text-sm text-muted-foreground">
                                특정 제독의 실제 소유주(유저)를 확인합니다. 대상에게도 알림이 전송됩니다.
                            </p>
                            <div className="flex gap-2">
                                <select
                                    className="flex-1 rounded-md border bg-background px-3 py-2 text-sm"
                                ...
                                <Button
                                    size="sm"
                                    disabled={!targetOwnerId || !info || info.points < actionCost.checkOwner}
                                    onClick={handleCheckOwner}
                                >
                                    소유자 찾기
                                </Button>
                            </div>
                            {ownerResult && <div className="text-sm p-2 rounded bg-muted">{ownerResult}</div>}
                        </CardContent>
                    </Card>
                </TabsContent>
