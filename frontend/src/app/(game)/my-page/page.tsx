...
export default function MyPage() {
    ...
    const [screenMode, setScreenMode] = useState<'auto' | 'pc' | 'mobile'>(() =>
        typeof window !== 'undefined'
            ? ((localStorage.getItem('sam_screenMode') as 'auto' | 'pc' | 'mobile') ?? 'auto')
            : 'auto'
    );
    ...
    const fi = frontInfo;
    const dexValues = [fi?.dex1 ?? 0, fi?.dex2 ?? 0, fi?.dex3 ?? 0, fi?.dex4 ?? 0, fi?.dex5 ?? 0];
    ...
    const penaltyEntries = Object.entries(g.penalty ?? {}).map(([key, value]) => {
                            ...
                            <CardContent className="pt-4 space-y-3">
                                <div className="flex gap-4 items-start">
                                    ...
                                    <div className="space-y-1 flex-1">
                                        ...
                                        {city && (
                                            <p className="text-sm">
                                                <span className="text-muted-foreground">위치:</span> {city.name}
                                            </p>
                                        )}
                                        <p className="text-sm">
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-2">
                                ...
                                <div className="flex items-center gap-2 text-sm">
                                    <span className="text-muted-foreground">귀속:</span> <span>{g.belong}</span>
                                    <span className="text-muted-foreground ml-3">배신:</span>{' '}
                                    <span className={g.betray > 0 ? 'text-red-400' : ''}>{g.betray}</span>
                                </div>
                            </CardContent>
                        </Card>
                    ...
                    <div className="grid gap-4 lg:grid-cols-2">
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="space-y-1">
                                    <label className="text-sm text-muted-foreground">수비 훈련도</label>
                                    <select
                                        value={defenceTrain}
                                ...
                                {/* Tournament */}
                                <div className="space-y-1">
                                    <label className="text-sm text-muted-foreground">토너먼트 참가</label>
                                    <select
                                ...
                                {/* Screen Mode */}
                                <div className="space-y-1">
                                    <label className="text-sm text-muted-foreground">화면 모드</label>
                                ...
                                {/* Potion Threshold */}
                                <div className="space-y-1">
                                    <label className="text-sm text-muted-foreground">약 사용 기준</label>
                                    <select
                        ...
                        <div className="space-y-4">
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-2">
                                    <Button variant="outline" onClick={handleVacation} className="w-full">
                                        휴가 전환
                                    </Button>
                                    <Button
                                        variant="outline"
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-2">
                                    <p className="text-xs text-muted-foreground">
                                        소지한 아이템을 파기합니다. 파기한 아이템은 복구할 수 없습니다.
                                    </p>
                                            ...
                                            return (
                                                <Button
                                                    ...
                                                    onClick={async () => {
                                                        ...
                                                        const isUnique =
                                                            (g.meta as Record<string, unknown>)?.[`${key}Unique`] ===
                                                                true ||
                                                            val?.includes('★') ||
                                                            val?.includes('유니크');
                                                        if (isUnique) {
                                                            ...
                                                            !confirm(`${label} [${val}]을(를) 정말 파기하시겠습니까?`)
                                                        )
                                                            return;
                                                        {
                                                            try {
                                                                ...
                                                                if (res.data.success) {
                                                                    toast.success(
                                                                        res.data.logs?.[0] ?? '아이템을 파기했습니다.'
                                                                    );
                                                                    router.refresh();
                                                                } else {
                                                                    toast.error(
                                                                        res.data.logs?.[0] ?? '파기에 실패했습니다.'
                                                                    );
                                                                }
                                                            } catch {
    ...
    return (
        <Tabs defaultValue="personal" value={activeLogTab} onValueChange={setActiveLogTab}>
            ...
            <TabsContent value="battle" className="mt-3">
                <Card>
                    <CardHeader>
                        <CardTitle className="text-sm">전투 기록 ({battleRecords.length}건)</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3">
                        ...
                        {oldLogsEnd && oldLogs.length > 0 && (
                            <p className="text-xs text-muted-foreground text-center py-2">모든 기록을 불러왔습니다.</p>
                        )}
...
function RecordList({ records, pageSize = 20 }: { records: Message[]; pageSize?: number }) {
    ...
    if (records.length === 0) {
        return <p className="text-sm text-muted-foreground py-4">기록이 없습니다.</p>;
    }
        ...
        <div className="space-y-2">
            {totalPages > 1 && (
                <div className="flex items-center gap-2 text-xs">
                    ...
                    {page > 0 && (
                        <Button size="sm" variant="ghost" className="h-6 px-2 text-xs" onClick={() => setPage(0)}>
                            처음으로
                        </Button>
                    )}
                </div>
            ...
            <ScrollArea className="max-h-[500px]">
                <div className="space-y-1">
                    {pagedRecords.map((r) => (
                        <div key={r.id} className="text-sm py-1.5 px-2 rounded hover:bg-muted/30">
                            ...
                            <span
                                dangerouslySetInnerHTML={{
                                    __html: (r.payload.content as string) ?? JSON.stringify(r.payload),
                                }}
                            />
                        </div>
                    ))}
