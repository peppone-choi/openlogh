...
const ALL_POLICY_KEYS = POLICY_CATEGORIES.flatMap((c) => c.fields.map((f) => f.key));
...
function DraggablePriorityList({
    ...
    const activeItems = activeKeys.map((k) => items.find((i) => i.key === k)).filter(Boolean) as DragItem[];
            ...
            <div className="space-y-3">
                ...
                <ul className="space-y-1">
                    ...
                    {activeItems.length === 0 && (
                        <div className="text-xs text-muted-foreground/60 py-2 text-center">활성 항목 없음</div>
                    )}
                </ul>
                ...
                {inactiveItems.length > 0 && (
                    <div className="space-y-1">
                        ...
                        {inactiveItems.map((item) => (
                            ...
                            >
                                <span className="w-5" />
                                <span className="flex-1 text-sm text-muted-foreground">{item.key}</span>
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <HelpCircle className="size-3 text-muted-foreground/50 shrink-0" />
                                    </TooltipTrigger>
...
export default function NpcPage() {
    ...
    const [serverNationPriorityItems, setServerNationPriorityItems] = useState<DragItem[] | null>(null);
    const [serverGeneralPriorityItems, setServerGeneralPriorityItems] = useState<DragItem[] | null>(null);
    ...
    const [defaultGeneralPriority, setDefaultGeneralPriority] = useState<string[]>([]);

    // General-level override state
    const [selectedNpcId, setSelectedNpcId] = useState<string>('');
    const [generalOverride, setGeneralOverride] = useState<Record<string, number>>({});
        ...
        npcPolicyApi
            ...
            .then(({ data }) => {
                ...
                if (Array.isArray(data.generalPriority)) {
                    ...
                    setPrevGeneralPriority([...(data.currentGeneralActionPriority as string[])]);
                ...
                if (Array.isArray(data.availableNationPriorityItems)) {
                    setServerNationPriorityItems(
                        (data.availableNationPriorityItems as string[]).map((k) => ({
                            key: k,
                            help: DEFAULT_NATION_PRIORITY_ITEMS.find((d) => d.key === k)?.help ?? k,
                        }))
                    );
                }
                if (Array.isArray(data.availableGeneralActionPriorityItems)) {
                    setServerGeneralPriorityItems(
                        (data.availableGeneralActionPriorityItems as string[]).map((k) => ({
                            key: k,
                            help: DEFAULT_GENERAL_PRIORITY_ITEMS.find((d) => d.key === k)?.help ?? k,
                        }))
                    );
                }
                ...
                if (Array.isArray(data.defaultGeneralActionPriority)) {
                    setDefaultGeneralPriority(data.defaultGeneralActionPriority as string[]);
                }
    ...
    const nationPriorityItems = serverNationPriorityItems ?? DEFAULT_NATION_PRIORITY_ITEMS;
    const generalPriorityItems = serverGeneralPriorityItems ?? DEFAULT_GENERAL_PRIORITY_ITEMS;
    ...
    function LastSetterInfo({ setter }: { setter?: { setter: string; date: string } }) {
                    ...
                    {policyLoading ? (
                        ...
                        <>
                                                ...
                                                return (
                                                    <div key={f.key} className="space-y-1">
                                                        ...
                                                        {zeroHint && displayValue === 0 && (
                                                            <p className="text-[10px] text-yellow-500/80">{zeroHint}</p>
                                                        )}
                                                    </div>
                                                );
                                            })}
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <Button variant="outline" size="sm" onClick={handleRollbackPolicy}>
                ...
                <TabsContent value="priority" className="mt-4 space-y-4">
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            ...
                            <p className="text-xs text-muted-foreground">
                                예턴이 없거나 지정되어 있더라도 실패하면 아래 순위에 따라 사령턴을 시도합니다.
                            ...
                            <div className="flex items-center gap-2 pt-2">
                                <Button size="sm" variant="outline" onClick={handleResetNationPriority}>
                                    <RotateCcw className="size-3 mr-1" />
                                    초깃값으로
                                </Button>
                                <Button size="sm" variant="outline" onClick={handleRollbackNationPriority}>
                                    <Undo2 className="size-3 mr-1" />
                                    이전값으로
                                </Button>
                            </div>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            ...
                            />
                            <div className="flex items-center gap-2 pt-2">
                                <Button size="sm" variant="outline" onClick={handleResetGeneralPriority}>
                                    <RotateCcw className="size-3 mr-1" />
                                    초깃값으로
                                </Button>
                                <Button size="sm" variant="outline" onClick={handleRollbackGeneralPriority}>
                                    <Undo2 className="size-3 mr-1" />
                                    이전값으로
                                </Button>
                            </div>
