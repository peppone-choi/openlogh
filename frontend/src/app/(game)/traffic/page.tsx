...
function TrafficBar({ value, maxValue, label }: { value: number; maxValue: number; label: string }) {
    ...
    return (
        <div className="flex items-center gap-2 h-8">
            ...
            <div className="flex-1 relative h-7 bg-muted/30 rounded overflow-hidden">
                ...
                <span
                    className={`relative z-10 text-xs font-mono leading-7 ${pct >= 10 ? 'float-right pr-2' : 'ml-2'}`}
...
export default function TrafficPage() {
    ...
    return (
        <div className="p-4 space-y-4 max-w-5xl mx-auto">
                    ...
                    <CardContent className="space-y-1">
                        {recentTraffic.length === 0 ? (
                            ...
                            recentTraffic.map((entry, idx) => {
                                ...
                                return (
                                    <TrafficBar key={idx} value={entry.refresh} maxValue={maxRefresh} label={label} />
                                );
                            })
                        )}
                    ...
                    <CardContent className="space-y-1">
                        {recentTraffic.length === 0 ? (
                            ...
                            recentTraffic.map((entry, idx) => {
                                const timeStr = entry.date ? entry.date.substring(11, 16) : '-';
                                const label = `${entry.year}년 ${entry.month}월 ${timeStr}`;
                                return <TrafficBar key={idx} value={entry.online} maxValue={maxOnline} label={label} />;
                            })
                        )}
                        <div className="text-center text-sm text-muted-foreground pt-2 border-t">
            ...
            <Card>
                <CardHeader className="pb-2">
                    <CardTitle className="text-lg text-center">주 의 대 상 자 (순간과도갱신)</CardTitle>
                </CardHeader>
                <CardContent className="space-y-1">
                    {/* Total row */}
                    <div className="flex items-center gap-2 h-5">
                        <span className="text-xs w-24 text-right shrink-0 font-semibold">접속자 총합</span>
                        <span className="text-xs w-28 text-center shrink-0">
                            {totalRefreshScoreTotal.toLocaleString()}({totalRefresh.toLocaleString()})
                        </span>
                    ...
                    {topRefreshers.map((user, idx) => {
                        const pct = totalRefresh > 0 ? Math.round((user.refresh / totalRefresh) * 1000) / 10 : 0;
                        const color = getTrafficColor(pct);
                        return (
                            <div key={idx} className="flex items-center gap-2 h-5">
                                <span className="text-xs w-24 text-right shrink-0">{user.name}</span>
                                <span className="text-xs w-28 text-center shrink-0">
                                    {user.refreshScoreTotal.toLocaleString()}({user.refresh.toLocaleString()})
                                </span>
                                <div className="flex-1 relative h-4 bg-muted/30 rounded overflow-hidden">
                                    {pct > 0 && (
                                        <div
