...
function parseEntry(msg: Message): HallEntry {
    ...
    return {
        ...
        scenario:
            (p.scenario as string) ?? (p.phase as string) ?? (p.description as string) ?? (p.content as string) ?? '',
                ...
                <div className="flex gap-2 overflow-x-auto pb-1">
                    {entries.slice(0, 10).map((entry, idx) => (
                        ...
                        >
                            ...
                            {/* Name */}
                            <div className="text-xs font-bold truncate w-full" style={{ color: entry.nationColor }}>
                                {entry.generalName}
                            </div>
                            {/* Owner */}
                            {entry.ownerName && (
