...
function formatTurnClock(turnTime?: string): string {
    if (!turnTime) return '';
    const hhmm = turnTime.match(/(\d{2}:\d{2})(?::\d{2})?/);
    if (hhmm) return hhmm[1];
    return turnTime.length >= 19 ? turnTime.slice(14, 19) : turnTime;
}

function parseReservedCommandBrief(brief?: string): string[] {
    if (!brief) return [];
        ...
        <div className="bg-blue-950/30 border border-blue-900/50 rounded-lg p-3 space-y-2">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 text-xs">
                ...
                <div className="text-center">
                    ...
                    <div className={`tabular-nums font-bold ${injuredCount > 0 ? 'text-orange-400' : ''}`}>
                        {injuredCount}명
                    </div>
                </div>
                <div className="text-center">
                    ...
                    {sorted.map((g) => {
                        ...
                        return (
                            <div key={g.id} className="flex items-center gap-2 text-xs relative">
                                ...
                                <span className="text-muted-foreground">{lastTurnAction || '대기'}</span>
                                {endTime && (
                                    <span
                ...
                <TooltipTrigger asChild>
                    <div className="flex items-center gap-3 rounded bg-muted/50 px-3 py-2 text-sm cursor-default hover:bg-muted/80 transition-colors">
                        ...
                        <div className="hidden sm:flex items-center gap-2 text-[10px] text-muted-foreground">
                            {isValidObjKey(g.flagshipCode) && (
                                <Badge variant="outline" className="text-[10px] px-1">
                                    기함
                                </Badge>
                            )}
                            {isValidObjKey(g.equipCode) && (
                                <Badge variant="outline" className="text-[10px] px-1">
                                    특수장비
                                </Badge>
                            )}
                            {isValidObjKey(g.engineCode) && (
                                <Badge variant="outline" className="text-[10px] px-1">
                                    기관
                                </Badge>
                            )}
                            {isValidObjKey(g.accessoryCode) && (
                                <Badge variant="outline" className="text-[10px] px-1">
                                    부속품
                                </Badge>
                            )}
    ...
    const handleCreate = async () => {
        ...
        try {
            await troopApi.create({
                ...
                leaderOfficerId: myGeneral.id,
                factionId: myGeneral.nationId,
                name: newName.trim(),
            });
            setNewName('');
            setShowCreate(false);
                        ...
                        <Card key={t.id}>
                            <CardHeader>
                                <CardTitle className="flex items-center justify-between">
                                    ...
                                    <div className="flex gap-1">
                                        ...
                                        {isLeader && (
                                            <Button size="sm" variant="destructive" onClick={() => handleDisband(t.id)}>
                                                해산
                                            </Button>
                                        )}
                                        {!isMember && (
                                        ...
                                        {isMember && !isLeader && (
                                            <Button size="sm" variant="outline" onClick={() => handleExit(t.id)}>
                                                탈퇴
                                            </Button>
                                        )}
                                    </div>
                            ...
                            <CardContent>
                                ...
                                {reservedCommands.length > 0 && (
                                    <div className="text-xs text-cyan-400 mb-2 bg-cyan-950/30 rounded px-2 py-1.5">
                                        <div className="flex items-center gap-1.5 mb-1">
                                            <FileText className="size-3 shrink-0" />
                                            <span>예약 명령</span>
                                        </div>
                                        <ul className="space-y-0.5 pl-1">
                                            {reservedCommands.map((command, idx) => (
                                                <li key={`${t.id}-${idx}`}>
                                                    {idx + 1}: {command}
                                                </li>
                                            ))}
