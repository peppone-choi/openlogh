...
import { officerApi, historyApi } from '@/lib/gameApi';
...
export default function GeneralDetailPage() {
    ...
    const [general, setGeneral] = useState<Officer | null>(null);
    ...
    const fetchData = useCallback(() => {
        ...
        Promise.all([officerApi.get(generalId), historyApi.getGeneralRecords(generalId).catch(() => ({ data: [] }))])
            .then(([genRes, recRes]) => {
    ...
    if (error) return <ErrorState title="제독 정보를 불러오지 못했습니다." onRetry={fetchData} />;
    if (!general)
        return (
            <div className="p-4">
                <EmptyState icon={User} title="제독를 찾을 수 없습니다." />
            </div>
        );
    ...
    return (
        <div className="p-4 space-y-6 max-w-3xl mx-auto">
            <PageHeader icon={User} title="제독 상세" />
