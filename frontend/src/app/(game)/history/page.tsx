...
const EVENT_VARIANT: Record<EventType, 'destructive' | 'default' | 'secondary' | 'outline'> = {
    war: 'destructive',
    diplomacy: 'default',
    nation: 'secondary',
    general: 'outline',
...
function classifyEvent(msg: Message): EventType {
    ...
    if (
        ...
        return 'war';
    if (t.includes('diplom') || text.includes('동맹') || text.includes('불가침') || text.includes('휴전'))
        return 'diplomacy';
    if (
        t.includes('nation') ||
        text.includes('건국') ||
    ...
    if (
        ...
        return 'general';
    if (t.includes('city') || text.includes('점령') || text.includes('함락') || text.includes('탈환')) return 'city';
    return 'other';
}

function getEventText(msg: Message): string {
    if (typeof msg.payload?.description === 'string') return msg.payload.description;
...
export default function HistoryPage() {
    ...
    const startYear = useMemo(() => {
        ...
        const fromMeta = currentWorld.meta['scenarioMeta'] as Record<string, unknown> | undefined;
    ...
    const loadByYearMonth = useCallback(async () => {
        ...
        try {
            const [historyRes, yearbookRes] = await Promise.all([
                historyApi.getWorldHistoryByYearMonth(currentWorld.id, selectedYear, selectedMonth),
                                ...
                                {mapHistory.length > 0 && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Button
                                            ...
                                            onClick={() =>
                                                setMapSnapshotIdx((i) => Math.min(mapHistory.length - 1, i + 1))
                                            }
                                        >
                                            다음 ▶
                                        </Button>
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        ...
                                        if (
                                            ...
                                            (y === currentWorld.currentYear && m <= currentWorld.currentMonth)
                                        ) {
                                            setSelectedYear(y);
                                            setSelectedMonth(m);
                                        }
                                ...
                                <div className="rounded-none border border-border p-3 space-y-2">
                                    ...
                                    <div className="space-y-1">
                                        {yearbook.nations.map((nation) => (
                                            ...
                                            >
                                                <span className="font-medium" style={{ color: nation.color }}>
                                                    {nation.name}
                                                </span>
                        ...
                        <CardContent className="space-y-3">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {(Object.keys(EVENT_LABELS) as EventType[]).map((type) => (
                                    <Badge
                                        key={type}
                                        variant={activeFilters.has(type) ? EVENT_VARIANT[type] : 'outline'}
                            ...
                            <div className="text-xs text-muted-foreground">총 {filteredEvents.length}건</div>
                        </CardContent>
                    </Card>
                                    ...
                                    <CardContent className="p-0">
                                        <div className="relative pl-6 border-l-2 border-muted ml-4">
                                            {events.map((event, idx) => (
                                                <div key={`${event.msg.id}-${idx}`} className="relative pb-3 last:pb-1">
                                                    {/* Dot */}
                                                    <div
