...
import { MessageSquare, Lock, Trash2, ChevronDown, ChevronUp, Send } from 'lucide-react';
...
function BoardPageContent() {
    ...
    useEffect(() => {
        ...
        const unsubMsg = subscribeWebSocket(`/topic/world/${currentWorld.id}/message`, () => {
            loadPublic();
            loadSecret();
        });
            ...
            >
                ...
                <TabsContent value={tab} className="mt-4 space-y-4">
                        ...
                        <Card>
                            <CardContent className="space-y-2">
                                ...
                                <Textarea
                                    ...
                                    placeholder={isSecret ? '기밀 내용을 입력하세요...' : '내용을 입력하세요...'}
                                ...
                                <div className="flex justify-end">
                                    <Button size="sm" onClick={handlePost} disabled={sending || !content.trim()}>
                                        {sending ? '작성 중...' : '작성'}
                                    </Button>
                                </div>
                            </CardContent>
                    ...
                    {loading ? (
                        ...
                        <EmptyState
                            icon={isSecret ? Lock : MessageSquare}
                            title={isSecret ? '기밀실에 글이 없습니다.' : '회의실에 글이 없습니다.'}
                        />
                    ) : (
...
function BoardRow({
    ...
    const preview = displayPreview.length > 50 ? displayPreview.slice(0, 50) + '...' : displayPreview;
        ...
        <div className="border-b border-gray-600/30">
            ...
            >
                ...
                <span className="truncate flex items-center gap-1 min-w-0">
                    ...
                    {!isSecret && senderNation && <NationBadge name={senderNation.name} color={senderNation.color} />}
                ...
                <span className="text-center truncate">{sender?.name ?? `#${post.srcId ?? '?'}`}</span>
                <span className="text-center text-muted-foreground text-[10px]">
                    {sentDate.getMonth() + 1}/{sentDate.getDate()} {sentDate.getHours().toString().padStart(2, '0')}:
                    ...
                    <Card>
                        <CardContent className="space-y-2">
                            <div className="flex items-center gap-2">
                                <GeneralPortrait picture={sender?.picture} name={sender?.name ?? '?'} size="sm" />
                            ...
                            <p className="text-xs whitespace-pre-wrap leading-relaxed">{formatLog(postContent)}</p>
                            ...
                            <div className="mt-3 pt-3 border-t border-gray-600/30 space-y-2">
                                ...
                                {comments.length > 0 ? (
                                    ...
                                    <p className="text-[10px] text-muted-foreground">댓글이 없습니다.</p>
                                )}
                                ...
                                {commentsLoading && (
                                    <div className="text-[10px] text-muted-foreground">댓글 불러오는 중...</div>
                                )}
                            </div>
                        </CardContent>
                    </Card>
