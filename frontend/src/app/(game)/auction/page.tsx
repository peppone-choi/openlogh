...
function isActive(payload: AuctionPayload): boolean {
    if (payload.status === 'completed' || payload.status === 'cancelled' || payload.finished) return false;
    if (payload.endTime && new Date(payload.endTime).getTime() <= Date.now()) return false;
    return true;
}
...
export default function AuctionPage() {
    ...
    const [finalizingAuctionId, setFinalizingAuctionId] = useState<number | null>(null);
    ...
    /* ── partition ── */
    const resourceAuctions = auctions.filter((a) => (p(a).type ?? 'resource') === 'resource');
                    ...
                    {showCreate && (
                        <Card>
                            ...
                            <CardContent className="space-y-3">
                                ...
                                <Button onClick={handleCreate} disabled={creating || !createAmount || !createStartBid}>
                                    {creating ? '등록 중...' : '등록'}
                                </Button>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-2">
                                <div className="text-xs text-muted-foreground">
                                    시세: 금 1 = 쌀 {marketPrice.ricePerGold.toFixed(3)} / 쌀 1 = 금{' '}
                                    {marketPrice.goldPerRice.toFixed(3)}
                                </div>
                                    ...
                                    <CardContent className="space-y-3 text-xs">
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                            ...
                                            <div>
                                                ...
                                                <span className={d.isCallerHost ? 'text-cyan-400 font-bold' : ''}>
                                                    ...
                                                    <div className="flex items-center gap-2 flex-wrap">
                                                        ...
                                                        <Button
                                                            ...
                                                            disabled={
                                                                ...
                                                                Number(bidAmounts[selectedAuction.id]) < minUniqueBid
                                                            }
                                                            className="h-8 text-xs"
                                                        >
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-center justify-center gap-1">
                                                    {a.id}
                                                    {isAnonymous && (
                                                        <span className="text-[9px] text-purple-400" title="익명 입찰">
                                                            🔒
                                                        </span>
                                                    )}
                                                </div>
                                                ...
                                                <div className={d.isCallerHost ? 'text-cyan-400 font-bold' : ''}>
                                                    {d.hostName ?? d.sellerName ?? '-'}
                                                </div>
                                                ...
                                                <div
                                                    className={
                                                        hb?.isCallerHighestBidder ? 'text-cyan-400 font-bold' : ''
                                                    }
                                                >
                                                    {isAnonymous
                                                        ? (d.obfuscatedName ?? '익명')
    ...
    return (
        <div className="space-y-2">
            ...
            {auctions.map((a) => {
                ...
                const bidder =
                    d.currentBidderName ??
                    (d.currentBidderId ? (genMap.get(d.currentBidderId)?.name ?? `#${d.currentBidderId}`) : '-');
                ...
                return (
                    <div key={a.id}>
                        ...
                        >
                            ...
                            <div className="tabular-nums">
                                {d.finishBidAmount ? `${bidUnitLabel} ${d.finishBidAmount.toLocaleString()}` : '-'}
                            </div>
...
function AuctionRow({
    ...
    return (
        <div className="border border-gray-700 rounded p-3 space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
                {showItem ? (
                    <span className="text-amber-400 font-bold text-sm">{d.itemName ?? d.item}</span>
                ) : (
                    <>
