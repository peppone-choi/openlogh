...
export default function MessagesPage() {
    ...
    const [recipientType, setRecipientType] = useState<ComposeRecipientType>('general');
    ...
    const unreadCountByTab = useMemo(() => {
        const countUnread = (msgs: Message[]) => msgs.filter((m) => typeof m.meta.readAt !== 'string').length;
        return {
    ...
    const handleMarkAsRead = async (id: number) => {
        try {
            ...
            setMessagesByTab((prev) => ({
                ...prev,
                [tab]: prev[tab].map((m) => (m.id === id ? { ...m, meta: { ...m.meta, readAt: now } } : m)),
            }));
        } catch {
            /* ignore */
        }
        ...
        <div className="p-4 space-y-4 max-w-3xl mx-auto">
                ...
                <Card className="border-amber-900/60 bg-zinc-950/70">
                    <CardContent className="space-y-3">
                        <div className="grid gap-3 md:grid-cols-2">
                            <div>
                                <label htmlFor="recipient-type" className="block text-xs text-amber-200/80 mb-1">
                                    수신 대상
                                </label>
                                <select
                                    id="recipient-type"
                            ...
                            <div>
                                <label htmlFor="mailbox-type" className="block text-xs text-muted-foreground mb-1">
                                    서신 종류
                                </label>
                                <select
                                    id="mailbox-type"
            ...
            <div className="space-y-2">
                {!canUseDiplomacy && tab === 'diplomacy' && (
                    <div className="text-xs text-amber-300">외교 서신은 관직 4 이상만 열람할 수 있습니다.</div>
                )}
                {refreshing && <div className="text-xs text-muted-foreground">서신을 갱신하고 있습니다...</div>}
                {inbox.length === 0 ? (
