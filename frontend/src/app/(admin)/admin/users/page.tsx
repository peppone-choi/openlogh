...
type UserActionPayload = {
    type: 'setAdmin' | 'removeAdmin' | 'delete' | 'setGrade' | 'block' | 'unblock' | 'extendOauth' | 'banPermanent';
    grade?: number;
    days?: number;
    oauthDays?: number;
};
...
export default function AdminUsersPage() {
    ...
    const load = useCallback(() => {
        adminApi
            ...
            .then((res) => {
                setUsers(res.data);
                setPendingGrades(Object.fromEntries(res.data.map((user) => [user.id, user.grade])));
            })
            .catch(() => {
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
                ...
                <Button
                    ...
                    variant="outline"
                    onClick={async () => {
                        if (!confirm('탈퇴 요청 후 1개월 이상 경과한 계정을 삭제합니다. 계속할까요?')) return;
                        try {
                ...
                <Button
                    ...
                    variant="outline"
                    onClick={async () => {
                        if (!confirm('1개월 이상 된 전투 아이콘 데이터를 정리합니다. 계속할까요?')) return;
                        try {
