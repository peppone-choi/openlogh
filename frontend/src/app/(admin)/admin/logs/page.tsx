...
const categoryColor: Record<CategoryKey, string> = {
    ...
    domestic: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
...
function LogEntry({ log, selected, onSelect }: { log: Message; selected: boolean; onSelect: (log: Message) => void }) {
    ...
    return (
        <Card className={cn('transition-colors', expanded && 'ring-1 ring-primary/20', selected && 'border-primary')}>
            <CardContent className="p-3">
                <div className="flex items-start gap-2 cursor-pointer" onClick={() => onSelect(log)}>
                    <div className="mt-0.5" onClick={(e) => e.stopPropagation()}>
                        {hasPayload ? (
                            <button type="button" onClick={() => setExpanded(!expanded)} className="inline-flex">
                                {expanded ? (
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                            <Badge variant="outline" className={cn('text-[10px] px-1.5 py-0', categoryColor[category])}>
...
export default function AdminLogsPage() {
    ...
    const filteredLogs = useMemo(() => {
        const byCategory =
            activeCategory === 'all' ? allLogs : allLogs.filter((log) => categorizeLog(log) === activeCategory);
    ...
    const pagedLogs = filteredLogs.slice((safePage - 1) * ITEMS_PER_PAGE, safePage * ITEMS_PER_PAGE);
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                <Button onClick={handleSearch} disabled={loading} className="bg-red-400 hover:bg-red-500 text-white">
                    조회
                </Button>
                {searched && (
                                ...
                                {selectedLog ? (
                                    <>
                                        <div className="text-xs space-y-1 text-muted-foreground">
                                            ...
                                            <p>
                                                메일박스: {selectedLog.mailboxCode} ({selectedLog.mailboxType})
                                            </p>
                                            <p>
                                                src: {selectedLog.srcId ?? '-'}, dest: {selectedLog.destId ?? '-'}
                                            </p>
