...
export default function RegisterPage() {
    ...
    const checkLoginIdDup = useCallback(async (value: string) => {
        ...
        try {
            const { data } = await api.post<{ result: boolean; reason?: string }>('/auth/check-dup', {
                field: 'username',
                value,
            });
            if (data.result) {
            ...
            <Card className="w-full max-w-md p-8">
                ...
                <CardContent className="px-0 pb-0">
                    ...
                    <div className="mb-5 space-y-2 border border-input rounded-md p-3">
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                            ...
                            <span className="text-sm flex-1">
                                이용약관 동의 <span className="text-destructive text-xs">(필수)</span>
                            </span>
                            <button
                                type="button"
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                            ...
                            />
                            <span className="text-sm flex-1">
                                개인정보 처리방침 동의 <span className="text-destructive text-xs">(필수)</span>
                            </span>
                            <button
                                type="button"
                                className="text-xs text-primary hover:underline"
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                            ...
                            />
                            <span className="text-sm flex-1">
                                개인정보 제3자 제공 동의 <span className="text-muted-foreground text-xs">(선택)</span>
                            </span>
                        </label>
                    </div>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        <div>
                            <label htmlFor="register-login-id" className="mb-1 block text-sm text-muted-foreground">
                                아이디
                            </label>
                            <Input
                        ...
                        <div>
                            <label htmlFor="register-display-name" className="mb-1 block text-sm text-muted-foreground">
                                닉네임
                            </label>
                            <Input
                            ...
                            {nickWidthError && <p className="mt-1 text-xs text-destructive">{nickWidthError}</p>}
                            {nickStatus === 'checking' && (
                                <p className="mt-1 text-xs text-muted-foreground flex items-center gap-1">
                                    <Loader2 className="size-3 animate-spin" /> 중복 확인 중...
                                </p>
                        ...
                        <Button type="submit" disabled={isSubmitting || !allAgreed} className="w-full">
