...
export default function LobbySelectNpcPage() {
    ...
    const validRemaining = token ? Math.max(0, Math.ceil((new Date(token.validUntil).getTime() - nowMs) / 1000)) : 0;
    ...
    const handleRefresh = async () => {
        ...
        try {
            ...
            setToken(data);
            setKeepIds((prev) => prev.filter((id) => data.npcs.some((n) => n.id === id)));
            toast.success(tokenExpired ? '새 NPC 카드를 생성했습니다.' : 'NPC 카드를 다시 뽑았습니다.');
        } catch (error) {
            const message =
                ...
                <>
                    <div className="flex flex-wrap items-center gap-3 rounded-lg border p-3 bg-card">
                        <div className="flex items-center gap-2 text-sm">
                            ...
                            <span className={validRemaining < 30 ? 'font-semibold text-red-500' : 'font-semibold'}>
                                {formatSeconds(validRemaining)}
                            </span>
                        </div>
                        <div className="text-sm text-muted-foreground">보존 {keepIds.length}/3</div>
                        <Button
                            type="button"
                            size="sm"
                            variant="outline"
                        ...
                        >
                            ...
                            {refreshRemaining > 0 ? `다시 뽑기 (${formatSeconds(refreshRemaining)})` : '다시 뽑기'}
                        </Button>
                    </div>
                                ...
                                <Card key={npc.id}>
                                    <CardContent className="space-y-3">
                                        <div className="flex items-center gap-3">
                                            <GeneralPortrait picture={npc.picture} name={npc.name} size="md" />
                                            <div className="min-w-0">
                                                ...
                                                >
                                                    {nationMap[npc.nationId]?.abbreviation || npc.nationName}
                                                </Badge>
                                            </div>
                                        </div>
                                        ...
                                        <label className="flex items-center gap-2 text-xs text-muted-foreground">
                                            <input
                                                type="checkbox"
                                                checked={kept}
                                                disabled={keepLocked || tokenExpired || selectingId !== null}
                                                onChange={(event) => handleKeepToggle(npc.id, event.target.checked)}
                                            />
                                            보존
                                        ...
                                        <Button
                                            ...
                                            disabled={tokenExpired || selectingId !== null || refreshing}
                                        >
                    ...
                    {tokenExpired ? (
                        <p className="text-sm text-red-500">
                            토큰 유효 시간이 만료되었습니다. 다시 뽑기를 눌러 새 카드를 생성하세요.
                        </p>
                    ) : null}
                </>
            )}
