...
describe('flattenAroundCities', () => {
    it('flattens terrain around city position', () => {
        const segX = 10;
        const segZ = 7;
        const heightMap = new Float32Array((segX + 1) * (segZ + 1));
        // Fill with mountain height
        heightMap.fill(3.0);

        const cities = [{ id: 1, x: 350, y: 250, name: 'A', level: 5 }] as unknown as Parameters<
            typeof flattenAroundCities
        >[3];

        flattenAroundCities(heightMap, segX, segZ, cities);

        // Center should be clamped (≤1.0)
        const cx = Math.round((350 / 700) * segX);
        const cz = Math.round((250 / 500) * segZ);
        const centerHeight = heightMap[cz * (segX + 1) + cx];
        expect(centerHeight).toBeLessThanOrEqual(1.0);
        expect(centerHeight).toBeGreaterThanOrEqual(0.2);
    });

    it('larger cities have wider flat area', () => {
        const segX = 20;
        const segZ = 14;
        const heightMap1 = new Float32Array((segX + 1) * (segZ + 1)).fill(2.0);
        const heightMap2 = new Float32Array((segX + 1) * (segZ + 1)).fill(2.0);

        const smallCity = [{ id: 1, x: 350, y: 250, name: 'S', level: 1 }] as unknown as Parameters<
            typeof flattenAroundCities
        >[3];
        const bigCity = [{ id: 1, x: 350, y: 250, name: 'B', level: 8 }] as unknown as Parameters<
            typeof flattenAroundCities
        >[3];

        flattenAroundCities(heightMap1, segX, segZ, smallCity);
        flattenAroundCities(heightMap2, segX, segZ, bigCity);

        // Count flattened vertices (height < 2.0)
        let flat1 = 0;
        let flat2 = 0;
        for (let i = 0; i < heightMap1.length; i++) {
            if (heightMap1[i] < 2.0) flat1++;
            if (heightMap2[i] < 2.0) flat2++;
        }
        expect(flat2).toBeGreaterThan(flat1);
    });
});
