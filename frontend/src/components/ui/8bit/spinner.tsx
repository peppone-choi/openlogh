...
type SpinnerProps = CommonSpinnerProps &
    ((React.ComponentProps<'svg'> & { variant?: 'classic' }) | (React.ComponentProps<'svg'> & { variant: 'diamond' }));

const Spinner = React.forwardRef<SVGSVGElement, SpinnerProps>(({ className, variant = 'classic', ...props }, ref) => {
    return (
        <>
            {variant === 'classic' && (
                <svg
