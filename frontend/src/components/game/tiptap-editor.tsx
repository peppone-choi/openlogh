...
const FONT_SIZES = ['8px', '10px', '12px', '14px', '18px', '22px', '28px', '36px', '48px', '72px'];
...
export function TipTapEditor({
    ...
    const colorConvert = useCallback((val: string | undefined, defaultVal: string) => {
        ...
        <div className={className}>
            {editable && (
                <div className="flex flex-wrap items-center gap-0.5 p-1 bg-muted rounded-t-md border border-b-0 border-border">
                    ...
                    {/* Colors */}
                    <ToolBtn tooltip="색상 취소" onClick={() => editor.chain().focus().unsetColor().run()}>
                    ...
                    <input
                        ...
                        value={colorConvert(editor.getAttributes('textStyle').color as string | undefined, '#ffffff')}
                    ...
                    {/* Image & HR */}
                    <ToolBtn tooltip="이미지 추가" onClick={() => setShowImageModal(true)}>
                        <ImageIcon className="h-4 w-4" />
                    </ToolBtn>
                    <ToolBtn tooltip="구분선" onClick={() => editor.chain().focus().setHorizontalRule().run()}>
            ...
            {showImageModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-background border border-border rounded-none p-6 w-full max-w-md space-y-4">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm text-muted-foreground">이미지 링크</label>
                            <Input
                                value={imageLink}
