...
interface OfficerLiteCardProps {
...
/**
 * Lightweight officer card showing portrait, name, stats, resources, and traits.
 * Ported from legacy GeneralLiteCard.vue.
 */
export function OfficerLiteCard({ general, nation, gameConst }: OfficerLiteCardProps) {
    ...
    const getActionInfo = (key: string, list?: Record<string, { name: string; info?: string }>) => {
        if (!key || !list || !list[key]) return { name: '-', info: '' };
        return list[key];
    };

    const personal = getActionInfo(general.personal, gameConst?.personalityList);
    const specialDomestic = getActionInfo(general.specialDomestic, gameConst?.specialDomesticList);
    const specialWar = getActionInfo(general.specialWar, gameConst?.specialWarList);
...
/** @deprecated Use OfficerLiteCard */
export { OfficerLiteCard as GeneralLiteCard };
        ...
        <TooltipProvider>
            <Tooltip>
                <TooltipTrigger asChild>
                    <span className="cursor-help underline decoration-dotted">{text}</span>
                </TooltipTrigger>
