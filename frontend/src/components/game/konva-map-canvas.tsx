...
interface KonvaMapCanvasProps {
    ...
    onCityHover?: (cityId: number | null, screenX: number, screenY: number) => void;
}
...
export default function KonvaMapCanvas({
    ...
    const constMap = useMemo(() => new Map(mapData.cities.map((c) => [c.id, c])), [mapData]);

    const { scaleX, scaleY, offsetX, offsetY } = useMemo(() => {
        if (mapData.cities.length === 0) return { scaleX: 1, scaleY: 1, offsetX: 0, offsetY: 0 };
        let minX = Infinity,
            maxX = -Infinity,
            minY = Infinity,
            maxY = -Infinity;
    ...
    const toX = useCallback((x: number) => (x + offsetX) * scaleX + PAD, [offsetX, scaleX]);
    const toY = useCallback((y: number) => (y + offsetY) * scaleY + PAD, [offsetY, scaleY]);
    ...
    const connections = useMemo(() => {
        ...
        for (const city of mapData.cities) {
            for (const connId of city.connections) {
                const key = city.id < connId ? `${city.id}-${connId}` : `${connId}-${city.id}`;
    ...
    const handleWheel = (e: Konva.KonvaEventObject<WheelEvent>) => {
        ...
        const scaleBy = 1.08;
        const newScale =
            e.evt.deltaY < 0 ? Math.min(oldScale * scaleBy, MAX_SCALE) : Math.max(oldScale / scaleBy, MIN_SCALE);
        const mousePointTo = {
        ...
        >
            ...
            <Layer listening={false}>
                ...
                {highlightedPaths.map((pts, i) => (
                    <Line key={`path-${i}`} points={pts} stroke="#ffaa00" strokeWidth={3} opacity={0.7} dash={[8, 4]} />
                ))}
            </Layer>
