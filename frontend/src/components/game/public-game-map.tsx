...
export function PublicGameMap({ data }: PublicGameMapProps) {
    ...
    const handleCityMouseEnter = useCallback((cityName: string, nationName: string, level: number) => {
    ...
    const mapCode = useMemo(() => (data.mapCode ?? 'che').trim() || 'che', [data.mapCode]);
    const mapFolder = mapCode.includes('miniche') ? 'che' : mapCode === 'ludo_rathowm' ? 'ludo_rathowm' : mapCode;

    const season = useMemo<MapSeason>(() => getSeason(data.currentMonth), [data.currentMonth]);
            ...
            >
                ...
                {data.currentYear != null && data.currentMonth != null && (
                    <div className="absolute top-0 left-0 right-0 z-[4] text-center py-1">
                        ...
                        >
                            西紀 {data.currentYear}年 {data.currentMonth}月 {SEASON_LABELS[season] ?? ''}
                        </span>
                    </div>
                )}
                ...
                <div className="absolute inset-0 z-[3]" onMouseMove={handleMouseMove}>
                    {data.cities.map((city) => {
                        ...
                        const top = city.y - 15;
                        const hasNation = city.nationColor && city.nationColor !== '#4b5563';
                        ...
                        return (
                            <div
                                ...
                                style={{ left, top }}
                                onMouseEnter={() => handleCityMouseEnter(city.name, city.nationName ?? '', city.level)}
