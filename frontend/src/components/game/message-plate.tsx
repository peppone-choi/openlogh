    ...
    return (
        <Card className="text-sm">
            <CardContent className="flex gap-2 py-2">
                <GeneralPortrait picture={senderGeneral?.picture} name={senderGeneral?.name ?? '시스템'} size="sm" />
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                        <span className="font-medium text-xs">
                            {senderGeneral?.name ?? `#${message.srcId ?? '시스템'}`}
                        </span>
                        <Badge variant={isSent ? 'outline' : 'secondary'} className="text-[10px]">
                            {isSent ? '발신' : '수신'}
                        </Badge>
                        {isDiplomacy && (
                    ...
                    <div className="flex gap-1 mt-1.5">
                        ...
                        {onReply && !isSent && message.srcId && (
                            <Button size="icon-sm" variant="ghost" onClick={() => onReply(message.srcId!)}>
