...
interface OfficerSupplementCardProps {
    general: GeneralFrontInfo;
    showCommandList?: boolean;
}

/**
 * Supplementary officer info card showing stats, dexterity levels,
 * and optionally reserved commands. Ported from legacy GeneralSupplementCard.vue.
 */
export function OfficerSupplementCard({ general, showCommandList = false }: OfficerSupplementCardProps) {
    ...
    const killRate =
        general.deathcrew > 0 ? ((general.killcrew / Math.max(general.deathcrew, 1)) * 100).toFixed(2) : '0.00';
    ...
    return (
        <div className="text-center text-sm border border-border rounded-lg overflow-hidden bg-card">
            ...
            <div className="grid grid-cols-6 bg-muted/30">
                ...
                <div className="border-b border-border">{general.bill.toLocaleString()}</div>

                <div className="bg-muted/50 border-b border-border">전투</div>
                <div className="border-b border-border">{general.warnum.toLocaleString()}</div>
                <div className="bg-muted/50 border-b border-border">계략</div>
                <div className="border-b border-border">{general.firenum.toLocaleString()}</div>
                ...
                <div className="border-b border-border">{general.killnum.toLocaleString()}</div>
                <div className="bg-muted/50 border-b border-border">패배</div>
                <div className="border-b border-border">{general.deathnum.toLocaleString()}</div>
                ...
                <div className="border-b border-border">{general.killcrew.toLocaleString()}</div>
                <div className="bg-muted/50 border-b border-border">피살</div>
                <div className="border-b border-border">{general.deathcrew.toLocaleString()}</div>
            </div>

            {/* Dexterity section */}
            <div className="flex">
                <div className={`${reservedCommands.length > 0 ? 'w-2/3' : 'flex-1'} grid grid-cols-4 bg-muted/30`}>
                    <div className="col-span-4 bg-muted/50 font-medium py-0.5">숙련도</div>
                    {dexList.map(([name, dex]) => {
                        const info = formatDexLevel(dex);
                        return (
                            <div key={name} className="contents">
                                <div className="bg-muted/50 border-b border-border">{name}</div>
                                <div className="border-b border-border" style={{ color: info.color }}>
                                    {info.name}
                                </div>
                ...
                {reservedCommands.length > 0 && (
                    <div className="w-1/3 bg-muted/30">
                        ...
                        {(reservedCommands as Record<string, unknown>[]).map((turn, idx: number) => (
                            <div key={idx} className="border-b border-border text-xs py-0.5 px-1">
                                {String(turn.brief ?? '')}
                            </div>
                        ))}
                    </div>
...
/** @deprecated Use OfficerSupplementCard */
export { OfficerSupplementCard as GeneralSupplementCard };
