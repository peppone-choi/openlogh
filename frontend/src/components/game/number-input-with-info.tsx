...
export function NumberInputWithInfo({
    ...
    const label = title ? <label className="text-sm text-muted-foreground whitespace-nowrap">{title}</label> : null;
    ...
    return (
        <div className={cn('space-y-0.5', className)}>
            ...
            {children && <p className="text-xs text-muted-foreground text-right">{children}</p>}
        </div>
    );
}
