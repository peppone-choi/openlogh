...
interface ShipClassBrowserProps {
    commandName: string; // 징병 or 모병
    onSubmit: (crewTypeCode: number, amount: number) => void;
}

export function ShipClassBrowser({ commandName, onSubmit }: ShipClassBrowserProps) {
    ...
    const myCity = useMemo(() => {
        ...
        return cities.find((c) => c.id === (myOfficer.starSystemId ?? myOfficer.cityId)) ?? null;
    }, [cities, myOfficer]);

    const techLevel = typeof myCity?.meta?.tech === 'number' ? (myCity.meta.tech as number) : 0;
    const leadership = myOfficer?.leadership ?? 100;
    const currentCrew = myOfficer?.ships ?? myOfficer?.crew ?? 0;
    const currentCrewType = myOfficer?.shipClass ?? myOfficer?.crewType ?? 0;
    const gold = myOfficer?.funds ?? myOfficer?.gold ?? 0;
    ...
    const totalCost = selectedCrew ? Math.ceil(amount * selectedCrew.cost * goldCoeff) : 0;
    const totalRice = selectedCrew ? Math.ceil(amount * selectedCrew.riceCost) : 0;
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-1 text-[10px] text-muted-foreground bg-zinc-900/50 rounded-md px-2 py-1.5">
                ...
                <div>
                    병사: <span className="text-amber-300 font-mono">{currentCrew.toLocaleString()}</span>
                </div>
                <div>
                    자금: <span className="text-amber-300 font-mono">{gold.toLocaleString()}</span>
                </div>
            </div>
                ...
                <div className="rounded-md border border-zinc-700 bg-zinc-900/50 p-2 space-y-2">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-1">
                            <Button variant="outline" size="sm" className="h-6 text-[10px] px-2" onClick={beHalf}>
                                절반
                            </Button>
                            <Button
                                variant="outline"
                            ...
                            <Button variant="outline" size="sm" className="h-6 text-[10px] px-2" onClick={beFull}>
                                가득
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-between text-[10px]">
                        ...
                        {totalCost > gold && <span className="text-red-400">자금 부족!</span>}
                    </div>

                    <Button size="sm" onClick={handleSubmit} disabled={amount <= 0} className="w-full">
...
/** @deprecated Use ShipClassBrowser */
export { ShipClassBrowser as CrewTypeBrowser };
export { CREW_TYPES };
