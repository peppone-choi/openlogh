...
Plans:
- [ ] 06-01-PLAN.md — gin7 도메인 타입 파일 생성 (Officer/Fleet/Planet/Faction/Command) + types/index.ts 재구성
- [ ] 06-02-PLAN.md — 은하맵 도트 아이콘 + 5단계 진영색 + 함대위치 마커 + 이동범위 하이라이트
- [ ] 06-03-PLAN.md — 전술맵 UI: △□◇ 유닛아이콘 + 커맨드레인지서클 + 미니맵 + 정보패널
- [ ] 06-04-PLAN.md — 전투 접근전 뷰: 상단 3열(아군패널+연출뷰+적군패널) + 하단 R3F 전술맵
- [ ] 06-05-PLAN.md — 전략게임 화면: 8스탯 패널 + 직무권한카드탭 + 커맨드실행 + 동스폿장교
- [ ] 06-06-PLAN.md — 함대/행성 관리: 함대편성 카드 + 행성자원 패널 + 창고이송 UI
- [ ] 06-07-PLAN.md — 정치 UI: 제국(쿠데타/귀족) + 동맹(의회/선거) + 페잔(차관/정보)
- [ ] 06-08-PLAN.md — 삼국지 레거시 UI 완전 제거 + 네비게이션 gin7 용어 정리
