...
### Completed: v2.1 전술전 지휘체계 + AI (2026-04-09, Phase 14 complete)
- Phase 08~13: 분함대 계층, 작전계획-발령, 커맨드레인지서클, 전술 AI, 지휘 승계, 전략 AI
- Phase 14: 프론트엔드 전술전 통합 — CommandRangeCircle 서버 주도, 5-레이어 Konva BattleMap, 서브플릿 드래그 드로어(@dnd-kit), 권한 게이팅 UI + Shift+클릭 제안, 승계 FX + 사운드, Fog-of-war 고스트(lastSeen + 60틱 페이드), 갤럭시 작전 오버레이(F1), 종전 모달(공적 내역 표시), R3F 전면 제거(Konva 단일 렌더러)
- v2.1 요구사항 FE-01..FE-05 모두 SATISFIED
...
---
*Last updated: 2026-04-09 after Phase 14 complete — v2.1 milestone done (FE-01..FE-05 validated). Known backend test debt: 205 pre-existing legacy 삼국지 tests in qa.parity/EconomyService/DiplomacyService/PlanetService/ScenarioService — queued for upstream-sync follow-up phase (reflecting OpenSamguk bugfixes). 9 human UX verification items pending in 14-HUMAN-UAT.md.*
