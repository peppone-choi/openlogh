...
import com.openlogh.gateway.config.JwtUtil
import com.openlogh.gateway.entity.AppUser
import com.openlogh.gateway.repository.AppUserRepository
import com.openlogh.shared.dto.AuthResponse
import com.openlogh.shared.dto.AutoLoginResponse
import com.openlogh.shared.dto.LoginByTokenRequest
import com.openlogh.shared.dto.LoginNonceResponse
...
import com.openlogh.shared.dto.UserInfo
...
class AuthService(
    ...
    fun login(request: LoginRequest): AuthResponse {
        ...
        val normalizedLoginId = normalizeLoginId(request.loginId)
        val user = appUserRepository.findByLoginId(normalizedLoginId)
    ...
    fun loginByToken(request: LoginByTokenRequest): AutoLoginResponse {
        ...
        val loginNonce = consumeMatchingNonce(tokenContext.token["baseToken"]?.toString(), request.hashedToken) ?: return AutoLoginResponse(
            result = false,
            silent = true,
            reason = "failed",
        )
        ...
        return try {
            ...
            appUserRepository.save(tokenContext.user)
    ...
    fun oauthLogin(provider: String, code: String, redirectUri: String): AuthResponse {
        ...
        ensureNotBlocked(user)
        upsertKakaoProvider(user, profile)
        val validUntil = handleKakaoOtpFlow(user, null)
        return loginUser(user, validUntil)
    }
    ...
    fun oauthRegister(
        ...
        if (existing != null) {
            ...
            upsertKakaoProvider(existing, profile)
            val validUntil = handleKakaoOtpFlow(existing, null)
            return loginUser(existing, validUntil)
        }
        ...
        checkDup("nickname", name)?.let { throw IllegalArgumentException(it) }

        val meta = mutableMapOf<String, Any>()
        agreeThirdUse?.let { meta["thirdUse"] = it }
        ...
        upsertKakaoProvider(user, profile)
        val saved = appUserRepository.save(user)
        val validUntil = handleKakaoOtpFlow(saved, null)
        return loginUser(saved, validUntil)
    }
    ...
    private fun refreshKakaoAccessToken(provider: MutableMap<String, Any?>, user: AppUser): MutableMap<String, Any?> {
        ...
        saveProviders(user, getProviderEntries(user).apply {
            replaceAll {
                if (it["provider"]?.toString()?.lowercase() == "kakao" &&
                    it["externalId"]?.toString() == provider["externalId"]?.toString()
                ) {
                    provider
                } else {
                    it
    ...
    private fun sendKakaoOtp(accessToken: String?, otpValue: String, validUntil: OffsetDateTime): Boolean {
        ...
        val template = mapper.writeValueAsString(
            mapOf(
                ...
                "link" to mapOf(
                    "web_url" to effectiveOtpLinkUrl(),
                    "mobile_web_url" to effectiveOtpLinkUrl(),
                ),
    ...
    private fun effectiveOtpLinkUrl(): String {
        if (appWebUrl.isNotBlank()) {
            return appWebUrl
        }
    ...
    private fun findUserByOAuth(provider: String, externalId: String): AppUser? {
        return appUserRepository.findAll().firstOrNull { user ->
            ...
            providers.any {
                ...
                p["provider"]?.toString()?.lowercase() == provider &&
                    p["externalId"]?.toString() == externalId
            }
        }
    }
    ...
    private fun upsertKakaoProvider(user: AppUser, profile: KakaoProfile): MutableMap<String, Any?> {
        val providers = getProviderEntries(user)
        val provider = providers.firstOrNull {
            it["provider"]?.toString()?.lowercase() == "kakao" &&
                it["externalId"]?.toString() == profile.id
        } ?: mutableMapOf<String, Any?>().also { providers.add(it) }

        provider["provider"] = "kakao"
        provider["externalId"] = profile.id
        provider.putIfAbsent("linkedAt", OffsetDateTime.now().toString())
        provider["accessToken"] = profile.accessToken
        provider["accessTokenValidUntil"] = profile.accessTokenValidUntil.toString()
        if (profile.refreshToken != null) {
            provider["refreshToken"] = profile.refreshToken
        }
        if (profile.refreshTokenValidUntil != null) {
            provider["refreshTokenValidUntil"] = profile.refreshTokenValidUntil.toString()
        }
    ...
    private fun ensureLoginAllowed() {
        val flags = systemSettingsService.getAuthFlags()
        if (!flags.allowLogin) {
    ...
    private fun checkUsernameDup(loginId: String): String? {
        ...
        if (appUserRepository.existsByLoginId(loginId)) return "이미 사용중인 계정명입니다"
        return null
    }
    ...
    private fun checkNicknameDup(displayName: String): String? {
        ...
        if (appUserRepository.existsByDisplayName(displayName)) return "이미 사용중인 닉네임입니다"
        return null
    }
    ...
    private fun findLoginTokenById(tokenId: Long): LoginTokenContext? {
        val now = OffsetDateTime.now()
        for (user in appUserRepository.findAll()) {
