...
/**
 * Fleet warehouse (부대창고): stores assigned units and supplies per fleet.
 * Patrol and Ground forces share a single pooled warehouse (not per-unit).
 * Fleet commanders draw from here for reorganization and replenishment.
 */
@Entity
@Table(
    name = "fleet_warehouse",
    uniqueConstraints = [UniqueConstraint(columnNames = ["session_id", "fleet_id"])]
)
class FleetWarehouse(
    ...
    // Ship counts by class (unit count)
