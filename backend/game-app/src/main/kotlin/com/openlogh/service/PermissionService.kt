...
/**
 * Manages ambassador/auditor permission assignments for a faction.
...
class PermissionService(
     ...
     * Only the faction leader (officer_level=20) can do this.
     *
     * @param userId the authenticated user making the request
     * @param nationId the faction being modified
     * @param isAmbassador true for ambassador (외교권자), false for auditor (감찰관)
     * @param generalIds list of officer IDs to assign the permission
     */
    @Transactional
    fun setPermission(userId: Long, nationId: Long, isAmbassador: Boolean, generalIds: List<Long>): PermissionResult {
        val requester = officerRepository.findByUserId(userId)
            .firstOrNull { it.factionId == nationId && it.npcState.toInt() == 0 }
        ...
        // Clear existing permissions of this type in the faction
        ...
        for (officer in nationGenerals) {
            if (officer.id !in assignableIds) {
                continue
            }
            officer.permission = targetType
