...
class CommandService(
    ...
    fun reserveGeneralTurns(generalId: Long, turns: List<TurnEntry>): List<OfficerTurn> {
        ...
        val world = sessionStateRepository.findById(general.worldId.toShort()).orElseThrow {
    ...
    fun executeCommand(generalId: Long, actionCode: String, arg: Map<String, Any>?): CommandResult? {
        ...
        val world = sessionStateRepository.findById(general.worldId.toShort()).orElse(null) ?: return null

        if (world.realtimeMode) {
            return realtimeService.submitCommand(generalId, actionCode, arg)
        }

        val city = planetRepository.findById(general.cityId).orElse(null)
        ...
        if (nation != null) factionRepository.save(nation)
        if (city != null) planetRepository.save(city)
        return result
    }
    ...
    fun getCommandTable(generalId: Long): Map<String, List<CommandTableEntry>>? {
        ...
        val world = sessionStateRepository.findById(general.worldId.toShort()).orElse(null) ?: return null
        val city = planetRepository.findById(general.cityId).orElse(null)
        val nation = if (general.nationId != 0L) factionRepository.findById(general.nationId).orElse(null) else null
        val env = createCommandEnv(world)
    ...
    fun repeatTurns(generalId: Long, count: Int): List<OfficerTurn>? {
        ...
        val queue = buildFullOfficerTurnQueue(general, existing, maxTurn)
        ...
        return replaceOfficerTurns(general, queue)
    }
    ...
    fun pushTurns(generalId: Long, amount: Int): List<OfficerTurn>? {
        ...
        val queue = buildFullOfficerTurnQueue(general, existing, maxTurn)
        val nextQueue = MutableList(maxTurn) { idx -> defaultOfficerTurn(general, idx.toShort()) }

        if (amount > 0) {
            for (idx in 0 until maxTurn) {
                nextQueue[idx] = if (idx < amount) {
                    defaultOfficerTurn(general, idx.toShort())
                } else {
        ...
        return replaceOfficerTurns(general, nextQueue)
    }

    private fun buildFullOfficerTurnQueue(general: com.openlogh.entity.Officer, existing: List<OfficerTurn>, maxTurn: Int): MutableList<OfficerTurn> {
        val indexed = existing.associateBy { it.turnIdx.toInt() }
        return MutableList(maxTurn) { idx ->
            indexed[idx]?.copyTurn(idx.toShort()) ?: defaultOfficerTurn(general, idx.toShort())
        }
    }

    private fun replaceOfficerTurns(general: com.openlogh.entity.Officer, turns: List<OfficerTurn>): List<OfficerTurn> {
    ...
    private fun defaultOfficerTurn(general: com.openlogh.entity.Officer, turnIdx: Short): OfficerTurn {
        return OfficerTurn(
            worldId = general.worldId,
    ...
    fun repeatNationTurns(generalId: Long, nationId: Long, count: Int): List<FactionTurn>? {
        ...
        val existing = factionTurnRepository.findByFactionIdAndOfficerLevelOrderByTurnIdx(general.nationId, general.officerLevel)
        if (existing.isEmpty()) return null
        ...
        val queue = buildFullFactionTurnQueue(general, existing, maxChiefTurn)
        ...
        return replaceFactionTurns(general, queue)
    }
    ...
    fun pushNationTurns(generalId: Long, nationId: Long, amount: Int): List<FactionTurn>? {
        ...
        val queue = buildFullFactionTurnQueue(general, existing, maxChiefTurn)
        val nextQueue = MutableList(maxChiefTurn) { idx -> defaultFactionTurn(general, idx.toShort()) }

        if (amount > 0) {
            for (idx in 0 until maxChiefTurn) {
                nextQueue[idx] = if (idx < amount) {
                    defaultFactionTurn(general, idx.toShort())
                } else {
        ...
        return replaceFactionTurns(general, nextQueue)
    }
    ...
    fun reserveNationTurns(generalId: Long, nationId: Long, turns: List<TurnEntry>): List<FactionTurn>? {
        ...
        val turnIdxList = turns.map { it.turnIdx }
        factionTurnRepository.deleteByFactionIdAndOfficerLevelAndTurnIdxIn(nationId, general.officerLevel, turnIdxList)
    ...
    private fun buildFullFactionTurnQueue(general: com.openlogh.entity.Officer, existing: List<FactionTurn>, maxChiefTurn: Int): MutableList<FactionTurn> {
        val indexed = existing.associateBy { it.turnIdx.toInt() }
        return MutableList(maxChiefTurn) { idx ->
            indexed[idx]?.copyFactionTurn(idx.toShort()) ?: defaultFactionTurn(general, idx.toShort())
        }
    }

    private fun replaceFactionTurns(general: com.openlogh.entity.Officer, turns: List<FactionTurn>): List<FactionTurn> {
        factionTurnRepository.deleteByNationIdAndOfficerLevel(general.nationId, general.officerLevel)
        return factionTurnRepository.saveAll(turns.map { it.copyFactionTurn(it.turnIdx) }).sortedBy { it.turnIdx }
    }

    private fun defaultFactionTurn(general: com.openlogh.entity.Officer, turnIdx: Short): FactionTurn {
        return FactionTurn(
            worldId = general.worldId,
    ...
    private fun createCommandEnv(world: com.openlogh.entity.WorldState): CommandEnv {
