...
class 집합(general: Officer, env: CommandEnv, arg: Map<String, Any>? = null)
    ...
    override suspend fun run(rng: Random): CommandResult {
        ...
        val movedMembers = mutableListOf<Officer>()
        val generalRepo = services?.officerRepository
        if (generalRepo != null && troopId > 0L) {
            val troopMembers = generalRepo.findByFleetId(troopId)
            for (member in troopMembers) {
                if (member.id != general.id && member.planetId != cityId) {
                    member.planetId = cityId
                    movedMembers.add(member)
                }
            }
            // Save moved members via destPlanetOfficers hook (CommandExecutor saves this list)
            if (movedMembers.isNotEmpty()) {
                destPlanetOfficers = movedMembers
            }
        }
