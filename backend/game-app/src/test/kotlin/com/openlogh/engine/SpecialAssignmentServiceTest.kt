...
class SpecialAssignmentServiceTest {
    ...
    fun `checkAndAssignSpecials skips assignment in first 3 years`() {
        ...
        val officer = createOfficer(age = 30, specAge = 20)

        service.checkAndAssignSpecials(world, listOf(officer))

        assertEquals("None", officer.specialCode, "Should not assign special in first 3 years")
    }

    @Test
    fun `checkAndAssignSpecials proceeds after 3 years`() {
        val world = createWorld(year = 187, startYear = 184)
        val officer = createOfficer(age = 30, specAge = 25, leadership = 80, intelligence = 80)

        service.checkAndAssignSpecials(world, listOf(officer))

        assertNotEquals("None", officer.specialCode, "Should assign special after 3 years")
    }
    ...
    fun `checkAndAssignSpecials assigns domestic special when age reaches specAge`() {
        ...
        assertNotEquals("None", officer.specialCode, "Should assign domestic special")
        assertTrue(officer.specAge > 30, "New specAge should be set for next assignment")
    }
    ...
    fun `checkAndAssignSpecials does not assign domestic special when age is below specAge`() {
        ...
        assertEquals("None", officer.specialCode, "Should not assign special when age < specAge")
    }
    ...
    fun `checkAndAssignSpecials skips generals who already have domestic special`() {
        ...
        assertEquals("농업", officer.specialCode, "Should not replace existing special")
    }
    ...
    fun `checkAndAssignSpecials assigns war special when age reaches spec2Age`() {
        ...
        assertNotEquals("None", officer.special2Code, "Should assign war special")
        assertTrue(officer.spec2Age > 30, "New spec2Age should be set for next assignment")
    }
    ...
    fun `checkAndAssignSpecials does not assign war special when age is below spec2Age`() {
        ...
        assertEquals("None", officer.special2Code, "Should not assign war special when age < spec2Age")
    }
    ...
    fun `checkAndAssignSpecials uses inherited war special when designated`() {
        ...
        assertEquals("필살", officer.special2Code, "Should use inherited war special")
        assertFalse(officer.meta.containsKey("inheritSpecificSpecialWar"), "Should remove meta key")
    }
    ...
    fun `calcStatCondition sets STAT_LEADERSHIP for high leadership`() {
        val officer = createOfficer(leadership = 80, command = 50, intelligence = 50)
        val result = service.calcStatCondition(officer)
    ...
    fun `calcStatCondition sets STAT_STRENGTH for high strength`() {
        val officer = createOfficer(leadership = 50, command = 80, intelligence = 50)
        val result = service.calcStatCondition(officer)
    ...
    fun `calcStatCondition sets STAT_INTEL for high intel`() {
        val officer = createOfficer(leadership = 50, command = 50, intelligence = 80)
        val result = service.calcStatCondition(officer)
    ...
    fun `calcStatCondition sets NOT flags for low stats when another stat is high`() {
        val officer = createOfficer(leadership = 30, command = 30, intelligence = 80)
        val result = service.calcStatCondition(officer)
    ...
    fun `calcDomesticSpecAge uses formula max of round 80 minus age div 12 minus relYear div 2 and 3 plus age`() {
        val result = service.calcDomesticSpecAge(30, 10)
    ...
    fun `calcDomesticSpecAge minimum wait is 3`() {
        val result = service.calcDomesticSpecAge(70, 50)
    ...
    fun `calcWarSpecAge uses formula max of round 80 minus age div 6 minus relYear div 2 and 3 plus age`() {
        val result = service.calcWarSpecAge(30, 10)
    ...
    fun `calcWarSpecAge minimum wait is 3`() {
        val result = service.calcWarSpecAge(70, 50)
    ...
    fun `checkAndAssignSpecials processes multiple generals`() {
        val world = createWorld(year = 200, startYear = 184)
        val g1 = createOfficer(id = 1, age = 30, specAge = 25, leadership = 80, intelligence = 80)
        val g2 = createOfficer(id = 2, age = 35, spec2Age = 30, command = 90)
        val g3 = createOfficer(id = 3, age = 20, specAge = 30)
