...
/**
 * TDD parity tests for WarUnitPlanet vs PHP WarUnitPlanet.php
 *
 * Legacy PHP reference: WarUnitPlanet::__construct()
...
class WarUnitCityParityTest {
    ...
    private fun buildCity(level: Short = 2): Planet = Planet(
        id = 1,
        sessionId = 1,
        name = "테스트도시",
        factionId = 2,
    ...
    fun `cityTrainAtmos is 60 at game start year plus 1`() {
        ...
        val unit = WarUnitPlanet(buildCity(), year = 181, startYear = 180)
        assertEquals(60, unit.training, "Train should equal cityTrainAtmos at game start")
        assertEquals(60, unit.morale, "Atmos should equal cityTrainAtmos at game start")
    }

    @Test
    fun `cityTrainAtmos is 80 at year 201 startYear 180`() {
        // PHP: clamp(201 - 180 + 59, 60, 110) = clamp(80, 60, 110) = 80
        val unit = WarUnitPlanet(buildCity(), year = 201, startYear = 180)
    ...
    fun `cityTrainAtmos caps at 110`() {
        ...
        val unit = WarUnitPlanet(buildCity(), year = 241, startYear = 180)
    ...
    fun `cityTrainAtmos with level 2 city has no bonus`() {
        val unit = WarUnitPlanet(buildCity(level = 2), year = 201, startYear = 180)
        assertEquals(80, unit.training)
        assertEquals(80, unit.morale)
    }
