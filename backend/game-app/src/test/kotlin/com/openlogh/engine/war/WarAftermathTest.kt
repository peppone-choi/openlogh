...
class WarAftermathTest {
    ...
    private fun buildPlanet(id: Long, factionId: Long): Planet {
        return Planet(
            id = id,
            sessionId = 1,
            name = "City$id",
    ...
    private fun buildFaction(id: Long): Faction {
        return Faction(
            id = id,
            sessionId = 1,
            name = "Faction$id",
    ...
    private fun buildOfficer(id: Long, factionId: Long, planetId: Long): Officer {
        return Officer(
            id = id,
            sessionId = 1,
            name = "Officer$id",
    ...
    fun `updates tech and diplomacy deltas`() {
        val attackerNation = buildFaction(1)
        val defenderNation = buildFaction(2)
        val attackerCity = buildPlanet(1, 1)
        val defenderCity = buildPlanet(2, 2)
        val attacker = buildOfficer(1, 1, 1)
    ...
    fun `applies conquest collapse rewards`() {
        val attackerNation = buildFaction(1)
        val defenderNation = buildFaction(2).apply {
            funds = 5000
            supplies = 6000
        }
        val attackerCity = buildPlanet(1, 1)
        val defenderCity = buildPlanet(2, 2).apply {
            meta["conflict"] = "{\"1\":100}"
        }
        val attacker = buildOfficer(1, 1, 1)
        val defender = buildOfficer(2, 2, 2)
