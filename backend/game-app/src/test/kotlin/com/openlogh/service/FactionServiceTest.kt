...
class FactionServiceTest {
    ...
    @Test
    fun `verifyPolicyAccess allows ambassador in same faction`() {
        val faction = Faction(id = 1, sessionId = 1, name = "위")
        ...
        `when`(officerRepository.findBySessionIdAndUserId(1L, 7L)).thenReturn(listOf(officer))
    ...
    fun `verifyPolicyAccess rejects penalized chief`() {
        ...
        `when`(officerRepository.findBySessionIdAndUserId(1L, 8L)).thenReturn(listOf(officer))
    ...
    fun `updateNotice stores legacy nationNotice envelope`() {
        ...
        val notice = faction.meta["nationNotice"] as Map<String, Any>
        assertEquals("새 방침", faction.meta["notice"])
    ...
    fun `updateNotice escapes html before persisting`() {
        ...
        assertEquals("&lt;script&gt;alert(1)&lt;/script&gt;", faction.meta["notice"])
        @Suppress("UNCHECKED_CAST")
        val notice = faction.meta["nationNotice"] as Map<String, Any>
    ...
    fun `updateBlockScout respects world lock flag`() {
        val faction = Faction(id = 1, sessionId = 1, name = "위", scoutLevel = 0)
        ...
        assertEquals(0.toShort(), faction.scoutLevel)
        verify(factionRepository, never()).save(faction)
    }
    ...
    fun `updateBlockWar decrements available count and updates war flag`() {
        ...
        assertEquals(1.toShort(), faction.warState)
        assertEquals(1, faction.meta["available_war_setting_cnt"])
