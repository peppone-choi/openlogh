...
- **우측 패널**: 적군 동일 구조
- **레이아웃**: 화면 전체가 하나의 3D 전투 장면, 좌우 상단에 정보 패널 오버레이
  - 좌상단: 아군 정보 (진영/제독/함대/부대/함종/함수/사기/무기)
  - 우상단: 적군 정보 (동일 구조)
  - 중앙: 3D 격자 전투 장면 (기함 모델 + 유닛 블록 + 무기 이펙트)
- **3D 전투 장면 구성**:
  - 검은 우주 배경 + 파란 격자 바닥
  - 아군(빨강 블록) ← → 적군(파랑 블록) 대치
  - 기함 = 3D 모델 (금색/고유 형상), 기함명 영문 표기 (HYPERION, KULISHUNA 등)
  - 유닛 블록 크기 = 함수에 비례
