...
- **나무위키 "은하영웅전설 4" 문서** — 전체 시스템 상세 (능력치, 시나리오, 제안시스템, 공적/계급, 요직, 전략커맨드 25종, 함대편성, 군함 8종, 전술모드, 행성/요새, 자금/경제, 쿠데타, 엔딩)
- 위키 전문을 참조하되, gin7 매뉴얼(PDF 101페이지)이 최우선
- gin4 EX와 gin7의 차이: gin4=싱글/턴제, gin7=온라인 MMO/리얼타임+직무권한카드+PCP/MCP
- gin4의 게임 시스템(장수제, 제안, 함대편성, 전술턴, 경제)이 gin7의 기반이므로 구현 시 반드시 참조
